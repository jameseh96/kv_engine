...
sharedPipeline(env) {
    ...
    envConf = {
        ...
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=Debug -DCOUCHBASE_KV_COMMIT_VALIDATION=1 -DCMAKE_C_COMPILER_EXTERNAL_TOOLCHAIN=/usr/local -DCMAKE_CXX_COMPILER_EXTERNAL_TOOLCHAIN=/usr/local"

        ENABLE_ADDRESSSANITIZER=1
        ENABLE_UNDEFINEDSANITIZER=1
        UBSAN_OPTIONS=print_stacktrace=1

        PATH="${env.PATH}:/usr/lib/llvm-3.9/bin/"
